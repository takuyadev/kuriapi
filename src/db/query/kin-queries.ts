...
export const getKinById = async (kinId: number, langId: number) => {
   try {
      const query = `
      SELECT
         a.id,
      ...
      // Await for response
      const data = await db.query(query, [kinId, langId]);
      return data.rows[0];
   } catch (err) {
      return err;
   }
...
export const getKinBySlug = async (slug: string, langId: number) => {
   try {
      const query = `
      SELECT
         a.id,
      ...
      // Await for response
      const data = await db.query(query, [slug, langId]);
      return data.rows[0];
   } catch (err) {
      return err;
   }
